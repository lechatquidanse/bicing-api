#@todo define availabilities
#@todo explain why this feature is a little more technical than it should be
Feature: Plan my itinerary requiring a station according to its past statistics availabilities
  In order to plan my itinerary requiring a station
  ...
  Scenario: Can't Retrieve last week availabilities from a station that does not exist
    Given station identified by "15cff96c-de06-4606-9870-b82eb9219339" with "station states from "last week":
      ...
      | -55 minutes 1 seconds | 7              | 23             | 'OPENED' |
    When I add "Accept" header equal to "application/ld+json"
    And I send a "GET" request to "/api/availabilities-by-station/station-id-that-does-not-exist"
